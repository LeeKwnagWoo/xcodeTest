...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, openURLContexts URLContexts: Set<UIOpenURLContext>) {
        if let windowScene = scene as? UIWindowScene {
            ...
            // rootViewController가 UINavigationController라면, 가장 위에 있는 ViewController를 가져옴
            var currentViewController = rootViewController
            while let presented = currentViewController?.presentedViewController {
                currentViewController = presented
            }

                // 현재 ViewController에서 Alert 표시
            let alert = UIAlertController(title: "알림", message: URLContexts.first?.url.absoluteString, preferredStyle: .alert)
